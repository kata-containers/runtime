...
HYPERVISOR_QEMU_VIRTIOFS_EXPERIMENTAL = qemu-virtiofs-experimental
...
HYPERVISORS := $(HYPERVISOR_ACRN) $(HYPERVISOR_FC) $(HYPERVISOR_QEMU) $(HYPERVISOR_QEMU_VIRTIOFS) $(HYPERVISOR_CLH) $(HYPERVISOR_QEMU_VIRTIOFS_EXPERIMENTAL)

QEMUPATH := $(QEMUBINDIR)/$(QEMUCMD)
QEMUVALIDHYPERVISORPATHS := [\"$(QEMUPATH)\"]

QEMUVIRTIOFSPATH := $(QEMUBINDIR)/$(QEMUCMD)
QEMUVALIDVIRTIOFSPATHS := [\"$(QEMUVIRTIOFSPATH)\"]

QEMUVIRTIOFSEXPERIMENTALPATH := $(QEMUBINDIR)/$(QEMUVIRTIOFSEXPERIMENTALCMD)
QEMUVALIDVIRTIOFSPATHS := [\"$(QEMUVIRTIOFSEXPERIMENTALPATH)\"]
...
DEFVIRTIOFSDAEMON_EXP := $(VIRTIOFSDBINDIR)/virtiofsd-experimental
...
ifneq (,$(QEMUVIRTIOFSEXPERIMENTALCMD))
    KNOWN_HYPERVISORS += $(HYPERVISOR_QEMU_VIRTIOFS_EXPERIMENTAL)

    CONFIG_FILE_QEMU_VIRTIOFS_EXP = configuration-qemu-virtiofs-experimental.toml
    CONFIG_QEMU_VIRTIOFS_EXP = $(CLI_DIR)/config/$(CONFIG_FILE_QEMU_VIRTIOFS_EXP)
    CONFIG_QEMU_VIRTIOFS_EXP_IN = $(CONFIG_QEMU_VIRTIOFS_EXP).in

    CONFIG_PATH_QEMU_VIRTIOFS_EXP = $(abspath $(CONFDIR)/$(CONFIG_FILE_QEMU_VIRTIOFS_EXP))
    CONFIG_PATHS += $(CONFIG_PATH_QEMU_VIRTIOFS_EXP)

    SYSCONFIG_QEMU_VIRTIOFS_EXP = $(abspath $(SYSCONFDIR)/$(CONFIG_FILE_QEMU_VIRTIOFS_EXP))
    SYSCONFIG_PATHS += $(SYSCONFIG_QEMU_VIRTIOFS_EXP)

    CONFIGS += $(CONFIG_QEMU_VIRTIOFS_EXP)
...
ifeq ($(DEFAULT_HYPERVISOR),$(HYPERVISOR_QEMU_VIRTIOFS_EXPERIMENTAL))
    DEFAULT_HYPERVISOR_CONFIG = $(CONFIG_FILE_QEMU_VIRTIOFS_EXP)
endif
...
USER_VARS += CONFIG_QEMU_VIRTIOFS_EXP_IN
USER_VARS += DESTDIR
...
USER_VARS += QEMUVIRTIOFSEXPERIMENTALPATH
...
USER_VARS += DEFVIRTIOFSDAEMON_EXP
...
ifneq (,$(findstring $(HYPERVISOR_QEMU_VIRTIOFS_EXPERIMENTAL),$(KNOWN_HYPERVISORS)))
	@printf "\t$(HYPERVISOR_QEMU_VIRTIOFS_EXPERIMENTAL) hypervisor path (QEMUVIRTIOFSEXPERIMENTALPATH) : %s\n" $(abspath $(QEMUVIRTIOFSEXPERIMENTALPATH))
endif
