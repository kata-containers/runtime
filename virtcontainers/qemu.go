...
var defaultKernelParameters = []hypervisor.Param{
	{"panic", "1"},
}
...
func (q *qemu) kernelParameters() string {
	...
	paramsStr := hypervisor.SerializeParams(params, "=")
...
func (q *qemu) Config() hypervisor.Config {
	return q.config
}
...
func (q *qemu) setup(id string, hypervisorConfig *hypervisor.Config, vcStore *store.VCStore) error {
	...
	nested, err := hypervisor.RunningOnVMM(hypervisor.ProcCPUInfo)
	if err != nil {
		return err
	}
...
func (q *qemu) CreateSandbox(ctx context.Context, id string, hypervisorConfig *hypervisor.Config, store *store.VCStore) error {
...
func (q *qemu) StartSandbox(timeout int) error {
	...
	if q.config.Debug {
		params := q.arch.kernelParameters(q.config.Debug)
		strParams := hypervisor.SerializeParams(params, "=")
...
func (q *qemu) hotplugAddBlockDevice(drive *config.BlockDrive, op hypervisor.Operation, devID string) error {
	var err error
...
func (q *qemu) hotplugBlockDevice(drive *config.BlockDrive, op hypervisor.Operation) error {
	err := q.qmpSetup()
	if err != nil {
		return err
	}
...
func (q *qemu) hotplugVFIODevice(device *config.VFIODev, op hypervisor.Operation) error {
	err := q.qmpSetup()
	if err != nil {
		return err
	}
...
func (q *qemu) hotplugDevice(devInfo interface{}, devType hypervisor.Device, op hypervisor.Operation) (interface{}, error) {
	switch devType {
...
func (q *qemu) HotplugAddDevice(devInfo interface{}, devType hypervisor.Device) (interface{}, error) {
	span, _ := q.trace("hotplugAddDevice")
	defer span.Finish()

	data, err := q.hotplugDevice(devInfo, devType, hypervisor.AddDevice)
	if err != nil {
		return data, err
	}
...
func (q *qemu) HotplugRemoveDevice(devInfo interface{}, devType hypervisor.Device) (interface{}, error) {
	span, _ := q.trace("hotplugRemoveDevice")
	defer span.Finish()

	data, err := q.hotplugDevice(devInfo, devType, hypervisor.RemoveDevice)
	if err != nil {
		return data, err
	}
...
func (q *qemu) hotplugCPUs(vcpus uint32, op hypervisor.Operation) (uint32, error) {
	if vcpus == 0 {
...
func (q *qemu) hotplugMemory(memDev *hypervisor.MemoryDevice, op hypervisor.Operation) (int, error) {
	...
	switch op {
	case hypervisor.RemoveDevice:
		memLog.WithField("hypervisor.Operation", "remove").Debugf("Requested to remove memory: %d MB", memDev.SizeMB)
		// Dont fail but warn that this is not supported.
		memLog.Warn("hot-remove VM memory not supported")
		return 0, nil
	case hypervisor.AddDevice:
		memLog.WithField("hypervisor.Operation", "add").Debugf("Requested to add memory: %d MB", memDev.SizeMB)
...
func (q *qemu) hotplugAddMemory(memDev *hypervisor.MemoryDevice) (int, error) {
...
func (q *qemu) AddDevice(devInfo interface{}, devType hypervisor.Device) error {
	var err error
...
func (q *qemu) resizeMemory(reqMemMB uint32, memoryBlockSizeMB uint32) (uint32, error) {
	...
	case currentMemory < reqMemMB:
		...
		addMemDevice := &hypervisor.MemoryDevice{
			SizeMB: int(memHotplugMB),
		}
		data, err := q.HotplugAddDevice(addMemDevice, hypervisor.MemoryDev)
		if err != nil {
			return currentMemory, err
		}
	...
	case currentMemory > reqMemMB:
		...
		data, err := q.HotplugRemoveDevice(addMemDevice, hypervisor.MemoryDev)
		if err != nil {
			return currentMemory, err
		}
...
func (q *qemu) GetThreadIDs() (*hypervisor.ThreadIDs, error) {
...
func (q *qemu) ResizeVCPUs(reqVCPUs uint32) (currentVCPUs uint32, newVCPUs uint32, err error) {
	...
	case currentVCPUs < reqVCPUs:
		...
		data, err := q.HotplugAddDevice(addCPUs, hypervisor.CPUDev)
		if err != nil {
	...
	case currentVCPUs > reqVCPUs:
		...
		data, err := q.HotplugRemoveDevice(removeCPUs, hypervisor.CPUDev)
		if err != nil {
			return currentVCPUs, newVCPUs, err
		}
