...
func (s *Sandbox) createContainers() error {
	...
	for _, contConfig := range s.config.Containers {
		s.Logger().Debugf("#### createContainers")
		c, err := newContainer(s, &contConfig)
		if err != nil {
			return err
		}
...
func (s *Sandbox) calculateSandboxCPUs() uint32 {
	...
	// FIXME: check
	for _, c := range s.containers {
		s.Logger().Debugf("#### calculateSandboxCPUs - cid %v", c.ID())
...
func (s *Sandbox) GetOCISpec() *specs.Spec {
	...
	spec, err := conf.GetOCISpec()
	if err != nil {
		s.Logger().WithError(err).Warnf("Sandbox without spec")
		return nil
	}
