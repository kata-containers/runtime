...
func (endpoint *TapEndpoint) Detach(netNsCreated bool, netNsPath string) error {
	...
	return doNetNS(netNsPath, func(_ ns.NetNS) error {
		return unTapNetwork(endpoint.Name())
	})
}
...
func tapNetwork(endpoint *TapEndpoint, numCPUs uint32, disableVhostNet bool) error {
	...
	tapLink, fds, err := createLink(netHandle, endpoint.TAPIface.Name, &netlink.Tuntap{}, int(numCPUs))
	if err != nil {
	...
	if !disableVhostNet {
		...
		if err != nil {
			return fmt.Errorf("Could not setup vhost fds %s : %s", endpoint.NicName, err)
		}
	...
	endpoint.TAPIface.HardAddr = linkAttrs.HardwareAddr.String()
	...
	if err := netHandle.LinkSetUp(tapLink); err != nil {
		return fmt.Errorf("Could not enable TAP %s: %s", endpoint.TAPIface.Name, err)
	}
	return nil
}
